...
public class Router {
    ...
    public static List<Long> shortestPath(GraphDB g,
        ...
        long idi = g.closest(stlon, stlat);
        long idf = g.closest(destlon, destlat);
        if (idi == idf) {
            return new LinkedList<>();
        }
        Map<Long, Double> path = new HashMap<>();
        ...
        class Compara implements Comparator<Long> {
            ...
            public int compare(Long o1, Long o2) {
                double comp1 = path.get(o1) + g.distance(o1, idf);
                double comp2 = path.get(o2) + g.distance(o2, idf);
                if (comp1 - comp2 < 0) {
                    return -1;
        ...
        PriorityQueue<Long> fringe = new PriorityQueue<>(new Compara());
        fringe.add(idi);
        while (!fringe.isEmpty()) {
            ...
            visited.add(id);
            for (long ids : g.adjacent(id)) {
                if (visited.contains(ids)) {
                    continue;
                }
                if (!fringe.contains(ids) && fringe.peek() != null) {
                    path.replace(ids, path.get(id) + g.distance(id, ids));
                    fringe.add(ids);
                    prev.put(ids, id);
                } else {
                    double distance = path.get(id) + g.distance(id, ids);
                    Compara t = new Compara();
                    if (distance < path.get(ids)) {
                        path.put(ids, distance);
                        prev.put(ids, id);
                        fringe.add(ids);
                    }
                }
        ...
        long iter = idf;
        while (path.get(iter) != 0.0) {
            shortestPath.addFirst(iter);
            if (prev.get(iter) == null) {
                break;
            }
            iter = prev.get(iter);
        }
        shortestPath.addFirst(idi);
        return shortestPath;
    }
